...
- **Movement:** per-enemy `MOVE_*` behavior — `STRAIGHT_LEFT` (default), `HOMING` (elites + rush packs), `SINE` (serpentine weave, sprinkled into the ambient stream), and `WALL` (slow advance + slide toward the player's row). Plus set-piece **formations** (`updateFormations`): rush packs (fast homing groups) and advancing walls, on a sim-time cadence (`FORMATIONS` in `data/waves.ts`, `MOVE_TUNING` in `data/enemies.ts`).
